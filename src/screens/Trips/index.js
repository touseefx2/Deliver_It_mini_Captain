...
function Trips(props) {
  ...
  const Sortdialog = () => {
    ...
    return (
      ...
      >
        <DialogContent
          style={{ padding: 15, paddingBottom: Platform.OS == "ios" ? 40 : 10 }}
        >
          <theme.Text
            style={{
...
{ds==sdt&&(
  <utils.vectorIcon.AntDesign name="check" color={theme.color.buttonLinerGC1} size={titlefz}/>
          ...
          >
            <theme.Text
              style={{
                color: ds == sta ? theme.color.buttonLinerGC1 : optioncolor,
                fontSize: optionfz,
                alignSelf: "center",
              }}
            >
          ...
          >
            <theme.Text
              style={{
                color: ds == std ? theme.color.buttonLinerGC1 : optioncolor,
                fontSize: optionfz,
                alignSelf: "center",
              }}
            >
  ...
  const showTrip = (e, i, a) => {
    ...
    let textcolor1 =
      e.status != "cancel" || e.cancelStatus == "unPaid"
        ? "black"
        : e.cancelStatus == "unPaidcut"
        ? "red"
        : "green";
    ...
    let myearn =
      e.status != "cancel"
        ? parseFloat(tcc - (cutPercent / 100) * tcc).toFixed(2)
        : e.cancelStatus == "unPaidcut"
        ? "- " + parseFloat(cutearn).toFixed(2)
        : e.cancelStatus == "Paid"
        ? "+ " + parseFloat(paidearn).toFixed(2)
        : "0.00";
  ...
  if (trip.length > 0) {
    ...
    trip.map((e, i, a) => {
      if (e.captainid == user.id && e.captaincarid == user.selectedCar) {
        if (
          (e.status == "cancel" && e.cancelby == "captain") ||
          e.endride == true ||
          (e.status == "skip" && e.cancelby == "captain") ||
          (e.status == "reject" && e.cancelby == "captain")
        ) {
          tripS.push(e);
        }
      }
