...
import hitApi from "../../database/ApiHelper";
...
function CaptainPortal(props) {
  ...
  const [startDate, setstartDate] = useState(
    moment().add(weeknum, "weeks").startOf("week")
  );
  const [endDate, setendDate] = useState(
    moment().add(weeknum, "weeks").endOf("week")
  );
  const [tcheck, settcheck] = useState(true);

  let earn = 0.0; //sara kch mila kr ktna kamaya deliver it ka tax cut kr or jo be cut ho ra   or jo extra kamaya wo be
  let acceptedTrips = 0;
  let totalTrips = 0;
  let totalcompletetrip = 0.0; //in a week
  let totalearn = 0.0; //total kamaya ktnacmplt trips without cuts tax
  let finaltotalearn = 0.0; //20 person deliverit ka   ktna hwa cmplt trips me se

  let totalCancelEarn = 0.0;
  ...
  const getTotal = async (sd, ed) => {
    ...
    db.api
      .apiCall(
        "get",
        db.link.gettotaltripCalculationwithDate + route,
        bodyData,
        header
      )
      .then((response) => {
  ...
  const getPortal = async (sd, ed) => {
    ...
    // method, path, body, header
    db.api
      .apiCall("get", db.link.getportalwithDate + route, bodyData, header)
      .then((response) => {
  ...
  const renderCycle = () => {
    let c = moment(endDate.format("YYYY-MM-DD")).isSameOrAfter(
      new Date(),
      "day"
    ); //check curent date is greater than last wekk end date or not
  ...
  const changeWeek = (c) => {
    totalTrips = 0;
    totalcompletetrip = 0.0; //in a week
    cancelTrips = 0.0;
    acceptedTrips = 0;

    earn = 0.0; //sara kch mila kr ktna kamaya deliver it ka tax cut kr or jo be cut ho ra   or jo extra kamaya wo be
    totalearn = 0.0; //20 person deliverit ka nikla kr
    finaltotalearn = 0.0; //total kamaya ktna cash me without cuts tax
    companyearn = 0.0;
    cutPercent = 20;
      ...
      return (
        <View>
          <TouchableOpacity
            style={{ width: "100%" }}
            disabled={
              e.status == "reject" ? true : e.status == "skip" ? true : false
            }
            onPress={() => {
              props.navigation.navigate("TripDetail", { e });
            }}
          >
            <View
              style={{
            ...
            >
              <View style={{ width: "57%" }}>
                ...
                <theme.Text
                  numberOfLines={1}
                  ellipsizeMode="tail"
                  style={{ fontSize: fontSize, color: "gray", lineHeight: 15 }}
                >
                  Trip {tid}
                </theme.Text>
                <theme.Text
...
<TouchableOpacity style={{width:"100%", paddingHorizontal:5,paddingVertical:10}}>
  ...
  if (portal && total) {
    ...
    totalCancelEarn =
      total.total_cancelled_trips_bycaptain +
      total.total_cancelled_trips_bycustomer;
    earn =
      parseInt(totalearn) +
      parseInt(finaltotalearn) +
      parseInt(totalCancelEarn);

    //captain rating
    captainRating =
      portal.rating != null ? portal.rating.toFixed(1) : (0).toFixed(1);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {!isInternet && !isserverErr && !loader && trip != false && (
