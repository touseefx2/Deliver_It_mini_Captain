...
function TripDetail(props) {
  ...
  const addDispute = (file) => {
    ...
    db.api
      .apiCall("post", db.link.addTripDispute, bodyData, header)
      .then((response) => {
        setl(false);
  ...
  const getDispute = () => {
    ...
    // method, path, body, header
    db.api
      .apiCall("get", db.link.getTripDispute + tid, bodyData, header)
      .then((response) => {
        setl(false);
        setisserverErr(false);
        ...
        if (response.data) {
          ...
          if (response.data.length > 0) setdispute(response.data[0]);
          else setdispute(response.data);
  ...
  const getTransaction = () => {
    ...
    db.api
      .apiCall("get", db.link.getTripTransctionHistory + tid, bodyData, header)
      .then((response) => {
  ...
  const SubmitClick = () => {
    Keyboard.dismiss();
    Alert.alert("Confirmation", "Are you sure you want to submit comment ?", [
      {
        text: "No",
          ...
          <View style={{ alignItems: "center", justifyContent: "center" }}>
            ...
            >
              <utils.vectorIcon.Ionicons
                ...
                size={65}
                color={!isrs ? theme.color.buttonLinerGC1 : "green"}
              />
            </TouchableOpacity>
          </View>
        )}
      ...
      return (
        ...
        >
          <View style={{ flex: 1, backgroundColor: "white" }}>
                ...
                >
                  <View style={{ width: "10%" }}>
                    <utils.vectorIcon.MaterialCommunityIcons
                      ...
                      size={25}
                      color={
                        sm == message1 ? theme.color.buttonLinerGC1 : "silver"
                      }
                    />
                  </View>
                  <theme.Text
                ...
                >
                  <View style={{ width: "10%" }}>
                    <utils.vectorIcon.MaterialCommunityIcons
                      ...
                      size={25}
                      color={
                        sm == message2 ? theme.color.buttonLinerGC1 : "silver"
                      }
                    />
                  </View>
                  <theme.Text
            ...
            <View style={{ padding: 10 }}>{renderButtonDispute()}</View>
          </View>
        </Modal>
      );
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView>
        ...
        {!l && dispute != "null" && !isserverErr && (
          <View style={{ padding: 10 }}>{renderdetail()}</View>
        )}
      </ScrollView>
