...
function Loader(props) {
  return (
    <MModal visible={props.loader} transparent={true} onRequestClose={() => {}}>
      <View
        style={{
          flex: 1,
...
function Profile(props) {
  ...
  const {
    regloading,
    isAllImageUploadDone,
    done,
    total,
    setisAllImageUploadDone,
    setdone,
    user,
    setRegLoading,
    setLoadingTrue,
  ...
  useEffect(() => {
    if (user) {
      ...
      setmobile(user.mobile_number.toString() || "");
      setName(user.fullname);
      setSelectedCity(user.city);
      setCNIC(user.cnic.toString());
      setEmail(user.email);
      setGender(user.gender || "");
      setCnicFrontImage(user.cnic_front_image);
      setCnicBackImage(user.cnic_back_image);
      setlicenseFrontImage(user.license_front_image);
      setlicenseBackImage(user.license_back_image);
      setAddress(user.address);

      let d = user.status == "approved" ? true : false;
      let dd = true;
      // let d = true;
      let e = false;
    ...
    Alert.alert("Confirmation", "Are you sure you want to update profile ?", [
      ...
      },
      {
        ...
        onPress: () => {
          let arr = [];
          if (image.uri) {
            arr.push({ c: "profile", image: image });
          }
          if (cnicFrontImage.uri) {
            arr.push({ c: "cnicF", image: cnicFrontImage });
          }
          if (cnicBackImage.uri) {
            arr.push({ c: "cnicB", image: cnicBackImage });
          }
          if (licenseFrontImage.uri) {
            arr.push({ c: "licenseF", image: licenseFrontImage });
          }
          if (licenseBackImage.uri) {
            arr.push({ c: "licenseB", image: licenseBackImage });
          }

          CheckIsImageChange(arr);
        },
      },
    ]);
  };

  const CheckIsImageChange = (arr) => {
    if (arr.length > 0) {
      try {
        let fa = [];
        setRegLoading(true);
        ...
        arr.map(async (e, i, a) => {
          ...
          await fetch(db.link.links + db.link.uploadFile, {
            method: "post",
            body: data,
            headers: {
            ...
            .then((responseData) => {
              setdone(done + 1);
              console.log("image uplaod done : ", e.c);
              fa.push({
                c: e.c,
                image: responseData.locationArray[0].fileLocation,
              });
            })
            .catch((err) => {
          ...
          if (i == a.length - 1) {
            if (fa.length > 0) {
              setTimeout(() => {
                setisAllImageUploadDone(true);
                let img = fa.find((x) => x.c == "profile")
                  ? fa.find((x) => x.c == "profile").image
                  : image;
                let cnicFimg = fa.find((x) => x.c == "cnicF")
                  ? fa.find((x) => x.c == "cnicF").image
                  : cnicFrontImage;
                let cnicBimg = fa.find((x) => x.c == "cnicB")
                  ? fa.find((x) => x.c == "cnicB").image
                  : cnicBackImage;
                let licenseFimg = fa.find((x) => x.c == "licenseF")
                  ? fa.find((x) => x.c == "licenseF").image
                  : licenseFrontImage;
                let licenseBimg = fa.find((x) => x.c == "licenseB")
                  ? fa.find((x) => x.c == "licenseB").image
                  : licenseBackImage;
        ...
        console.log("image upload error try catch : ", err);
        setRegLoading(false);
      }
    } else {
      setLoadingTrue();
  ...
  const onclickImage = (c) => {
    ...
    if (c == "cnicFV") {
      setpv(cnicFrontImage.uri || cnicFrontImage);
      setpvm(true);
      return;
    }

    if (c == "cnicBV") {
      setpv(cnicBackImage.uri || cnicBackImage);
      setpvm(true);
      return;
    }
                  ...
                  <>
                    ...
                    >
                      ...
                      <View style={styles.FormTitle1}>
                        <View style={{ width: "47%" }}>
                          <View
                            style={!fnDisable ? styles.InputT : styles.InputTD}
                          >
                        ...
                        <View style={{ width: "47%" }}>
                          <TouchableOpacity
                            ...
                            activeOpacity={0.4}
                            style={!cityDisable ? styles.BoxT : styles.BoxTD}
                          >
                    ...
                    <View style={styles.Form}>
                      ...
                      <View style={styles.FormTitle1}>
                        ...
                        >
                          <View
                            style={
                              !cnicDisable ? styles.InputT : styles.InputTD
                            }
                          >
                        ...
                        <View style={{ width: "47%" }}>
                          ...
                          {isEmptyMobile && renderShowError("mobile")}
                          {invalidMobile && renderShowError("mobile")}
                        </View>
                      </View>
                    </View>
                    ...
                    <View style={styles.cnicForm}>
                      <View style={styles.cnicImgConatiner}>
                            ...
                            >
                              ...
                              {cnicFrontImage != null && (
                                <ProgressiveFastImage
                                  ...
                                  loadingImageStyle={styles.pfImageLoader}
                                  loadingSource={require("../../assets/imgLoader.gif")}
                                />
                              )}
                            </TouchableOpacity>
                          </View>

                          {cnicFrontImage != null && !cnicImgDisable && edit && (
                            <TouchableOpacity
                              onPress={() => {
                        ...
                        <View style={{ width: "47%" }}>
                            ...
                            >
                              ...
                              {cnicBackImage != null && (
                                <ProgressiveFastImage
                                  source={{
                                    uri: normalisedSource(
                                      cnicBackImage.uri || cnicBackImage
                                    ),
                                  }}
                                  style={styles.cnicImage}
                                  loadingImageStyle={styles.pfImageLoader}
                          ...
                          {cnicBackImage != null && !cnicImgDisable && edit && (
                            <TouchableOpacity
                              onPress={() => {
                            ...
                            >
                              ...
                              {licenseFrontImage != null && (
                                <ProgressiveFastImage
                                  source={{
                                    uri: normalisedSource(
                                      licenseFrontImage.uri || licenseFrontImage
                                    ),
                                  }}
                                  style={styles.cnicImage}
                                  loadingImageStyle={styles.pfImageLoader}
                            ...
                            >
                              ...
                              {licenseBackImage != null && (
                                <ProgressiveFastImage
                                  source={{
                                    uri: normalisedSource(
                                      licenseBackImage.uri || licenseBackImage
                                    ),
                                  }}
                                  style={styles.cnicImage}
                                  loadingImageStyle={styles.pfImageLoader}
                  ...
                  <>
                    ...
                    >
                      ...
                      <View style={styles.FormTitle1}>
                        <View style={{ width: "47%" }}>
                          <View
                            style={!fnDisable ? styles.Input : styles.InputD}
                          >
                            <TextInput
                              editable={!fnDisable}
                              value={name}
                    ...
                    <View style={styles.Form}>
                      ...
                      <View style={styles.FormTitle1}>
                        ...
                        >
                          <View
                            style={!cnicDisable ? styles.Input : styles.InputD}
                          >
                            <TextInput
                            ...
                            >
                              ...
                              {cnicFrontImage != null && (
                                <Image
                                  style={styles.cnicImage}
                                  source={{
                                    uri: cnicFrontImage.uri || cnicFrontImage,
                                  }}
                                />
                              )}
                            </TouchableOpacity>
                            ...
                            >
                              ...
                              {cnicBackImage != null && (
                                <Image
                                  style={styles.cnicImage}
                                  source={{
                                    uri: cnicBackImage.uri || cnicBackImage,
                                  }}
                                />
                              )}
                            </TouchableOpacity>
