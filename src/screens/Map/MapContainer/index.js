...
function MapContainer(props) {
  ...
  useEffect(() => {
    if (accept && !mr && !arrive && request) {
      ...
      mapRef?.current?.fitToCoordinates([mark1, mark2], {
        animated: true,
        edgePadding: { top: 300, right: 100, bottom: 300, left: 100 },
      });
    }
  ...
  const getcustomerWalletinfo = () => {
    ...
    db.api
      .apiCall("get", db.link.getcustomerWalletinfo + cid, bodyData, header)
      .then((response) => {
        setll(false);
  ...
  useEffect(() => {
    if (search != "") {
      ...
      mapRef?.current?.fitToCoordinates([mark1, mark2], {
        animated: true,
        edgePadding: { top: 150, right: 120, bottom: 150, left: 120 },
      });
    } else {
      gotoCurrentLoc();
    }
  ...
  const fetchDistanceBetweenPointsOnline = (lat1, lng1, lat2, lng2, c) => {
    ...
    if (c == "requestride") {
      fetch(urlToFetchDistance)
        ...
        .then((res) => {
          ...
          return; //distance  captain curent loc  to user  pickup loc
        })
        .catch((error) => {
    ...
    } else if (c == "startridedynamic") {
      try {
        console.log("strt ride dynmc callllll");
        fetch(urlToFetchDistance)
          .then((res) => {
            return res.json();
          })
        ...
        console.log(
          "start ride dynamic fetchDistanceBetweenPointsOnline api error ",
          error
        );
      }
    } else if (c == "endride") {
      try {
        ...
        setl(false);
        console.log(
          "end ride fetchDistanceBetweenPointsOnline api error ",
          error
        );
      }
    }
  ...
  const onClickAccept = () => {
    if (isInternet) {
      ...
      // method, path, body, header
      db.api
        .apiCall("put", db.link.acceptTrip + request._id, bodyData, header)
        .then((response) => {
  ...
  const onClickArrive = () => {
    if (isInternet) {
      ...
      // method, path, body, header
      db.api
        .apiCall("put", db.link.arriveTrip + request._id, bodyData, header)
        .then((response) => {
  ...
  const onClickStart = () => {
    ...
    // method, path, body, header
    db.api
      .apiCall("put", db.link.startTrip + request._id, bodyData, header)
      .then((response) => {
  ...
  const canceltripYes = () => {
    if (isInternet) {
      ...
      if (accept && !arrive) {
        onClickcancelTrip(
          "Emergency (Canceling before arrive)",
          sec,
          cf == 0 ? cf : -cf
        );
        return;
      ...
      if (arrive && !startride) {
        onClickcancelTrip(
          "Emergency (Canceling after arrive brfore gracetime)",
          sec,
          cf == 0 ? cf : -cf
        );
        return;
      ...
      if (startride) {
        onClickcancelTrip(
          "Emergency (Canceling after startride)",
          sec,
          cf == 0 ? cf : -cf
        );
        return;
  ...
  const cancelTrip = () => {
    Alert.alert("Confirmation", "Are you sure you want to cancel this trip ?", [
      {
        text: "No",
        onPress: () => console.log("Cancel Pressed"),
        style: "cancel",
  ...
  const cancelJob = () => {
    Alert.alert("Confirmation", "Are you sure you want to cancel this job ?", [
      {
        text: "No",
        onPress: () => console.log("Cancel Pressed"),
        style: "cancel",
  ...
  const cancelJobYes = () => {
    ...
    if (isInternet) {
      ...
      onClickcancelTrip(
        "Emergency (Canceling after arrive after gracetime)",
        sec,
        +ctcfaa
      );
    } else {
  ...
  const clickArrivePickup = () => {
    Alert.alert(
      "Confirmation",
      "Are you sure you have arrived at pickup location ?",
      [
        {
          text: "No",
          onPress: () => console.log("Cancel Pressed"),
  ...
  const clickStartRide = () => {
    Alert.alert("Confirmation", "Do you want to start the ride ?", [
      {
        text: "No",
        onPress: () => console.log("Cancel Pressed"),
        style: "cancel",
  ...
  const onCashsubmit = (c, ra) => {
    ...
    if (c == "cutt") {
      ...
      db.api
        .apiCall("put", db.link.paycashLess + request._id, bodyData, header)
        .then((response) => {
    ...
    if (c == "add") {
      ...
      db.api
        .apiCall("put", db.link.paycashExtra + request._id, bodyData, header)
        .then((response) => {
  ...
  const confirmCashSubmit = (c, ra) => {
    Alert.alert(
      "Confirmation",
      "Are you sure you want to submit this amount?",
      [
        {
          text: "No",
          onPress: () => console.log("Cancel Pressed"),
        ...
        >
          ...
          >
            ...
            <theme.Text
              numberOfLines={1}
              ellipsizeMode="tail"
              style={{ fontSize: 14, color: "gray", lineHeight: 20 }}
            >
              {title2}
            </theme.Text>
          </View>
          ...
          >
            <Image
              source={require("../../../assets/Navigate/navigate.png")}
              ...
              {waitTime != "f" && (
                <CountDown
                  ...
                  digitStyle={{
                    backgroundColor: "#FFF",
                    borderWidth: 0,
                    borderColor: theme.color.buttonLinerGC1,
                  }}
          ...
          >
            ...
            <Input
              ...
              onChangeText={(nextValue) => {
                setcsh(nextValue.replace(/\D/gm, ""));
                setcash(parseInt(nextValue));
              }}
            />
          </View>

          <TouchableOpacity
            disabled={csh == "" ? true : false}
            onPress={() => {
              clickCashSubmit();
            }}
            style={[styles.BottomButton, { width: "100%" }]}
        ...
        >
          ...
          >
            ...
            <View style={{ marginLeft: 10, width: "85%" }}>
              ...
              >
                You can view your earnings in captain portal.
              </theme.Text>
            </View>
          </View>
        ...
        >
          ...
          <ScrollView contentContainerStyle={{ flex: 1 }}>
            ...
            >
              ...
              <TouchableOpacity
                onPress={() => {
                  callUser(request.customer.mobile_number);
                }}
              >
                <View
                  style={{
                    ...
                    alignItems: "center",
                    justifyContent: "center",
                    backgroundColor: theme.color.buttonLinerGC1,
                    borderRadius: 30,
                  }}
                >
                ...
                >
                  ...
                  >
                    ...
                    <Text
                      numberOfLines={1}
                      ellipsizeMode="tail"
                      style={{ fontSize: 14, lineHeight: 20, top: 2 }}
                    >
                      {ar.toFixed(1)}
                    </Text>
                    <utils.vectorIcon.Entypo
    ...
    return (
      ...
      >
        ...
        >
          ...
          <Text style={{ fontSize: 18, color: "#0e47a1", fontWeight: "bold" }}>
            {cashG
              ? "Collected cash amount is greater than fare rupees"
              : "Collected cash amount is less than fare rupees"}
          </Text>
              ...
              >
                ...
                >
                  <Text
                    style={{
                      fontSize: 15,
                      color: !checkBox ? "silver" : "black",
                    }}
                  >
                    Add extra amount in user wallet
                  </Text>
                </TouchableOpacity>
              </View>
                  ...
                  {cw !== "f" && cw !== "false" && !ll && (
                    <View>
                      ...
                      >
                        <LinearGradient
                          colors={[
                            theme.color.buttonLinerGC1,
                            theme.color.buttonLinerGC2,
                          ]}
                          style={styles.LinearGradient}
                        >
                          <View style={[styles.ButtonRight, { width: "100%" }]}>
            ...
            <View>
              ...
              >
                ...
                >
                  ...
                  >
                    Cut remaining amount from user wallet
                  </Text>
                </TouchableOpacity>
              </View>
                    ...
                    <View>
                      ...
                      <View style={{ marginTop: 10 }}>
                        ...
                        {uwta >= ra ? (
                          ...
                          >
                            Sorry user have no wallet amount
                          </Text>
                        )}
                      </View>
                    </View>
  ...
  const destinationMarker = () => {
    return (
      <Marker
        ...
        pinColor={theme.color.buttonLinerGC1}
      >
        <TouchableOpacity
          style={{
            width: 25,
    ...
    <SafeAreaView style={{ flex: 1 }}>
      ...
      >
        ...
        {accept &&
          ...
          !endride &&
          request &&
          dropoffMarker(request.dropoff.location)}
      </MapView>
      ...
      {!accept && (
        <Header
          setLoader={(c) => setl(c)}
          setActiveChecked={(t) => setActiveChecked(t)}
          propsH={props.propsH}
        />
      )}
      {!accept && (
        <SearchBox
          gotoCurrentLoc={() => gotoCurrentLoc()}
          Search={search}
          accept={accept}
          propsH={props.propsH}
          setSearch={(t) => setsearch(t)}
        />
      )}
